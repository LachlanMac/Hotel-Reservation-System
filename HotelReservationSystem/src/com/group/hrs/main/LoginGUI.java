...
public class LoginGUI extends JFrame implements ActionListener {
	...
	private void createButtons() {
		...
		submit.addActionListener((ActionEvent event) -> {
			...
			if (!DatabaseLoader.accountIsLocked(username) && Authenticator.validate(username, password)) {
				DatabaseLoader.resetAttempts(username);
				new ReservationGUI();
				frame.dispose();
			} else if (DatabaseLoader.accountIsLocked(username)) {
				JOptionPane.showMessageDialog(this, "User locked out.  Please contact System Admin");
			} else if (!Authenticator.validate(username, password) && !DatabaseLoader.accountIsLocked(username)) {

				DatabaseLoader.failedLoginAttempt(username);
				JOptionPane.showMessageDialog(this, "Please insert valid login username and password.");
			} else {
				JOptionPane.showMessageDialog(this, "Unchecked Error.");
